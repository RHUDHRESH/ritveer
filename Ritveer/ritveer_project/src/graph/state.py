...
class RitveerState(TypedDict, total=False):
    ...
    - messages: transcript (list of messages)
    - profile: user profile
    - story: request story
    - bom: bill of materials
    - suppliers: list of suppliers
    - quote: current quote
    - events: list of events [{ts, agent, type, data}]
    - artifacts: generated artifacts
    - policy: {thresholds, gates, copy}
    - agents: {AgentName: {status, output, ms, reasons?}}
    ...
    """
    # Shared state contract
    messages: List[dict[str, Any]]  # transcript
    profile: Optional[dict[str, Any]]
    story: Optional[dict[str, Any]]
    bom: Optional[dict[str, Any]]
    suppliers: Optional[List[dict[str, Any]]]
    quote: Optional[dict[str, Any]]
    events: List[dict[str, Any]]  # [{ts, agent, type, data}]
    artifacts: Optional[dict[str, Any]]
    policy: dict[str, Any]  # thresholds, gates, copy
    agents: dict[str, dict[str, Any]]  # {AgentName: {status, output, ms, reasons?}}
